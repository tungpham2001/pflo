const ProjectBoxLeft = ({ x, y, text, techStacks, backgroundImage}) => {
    return (
      ...
      >
        ...
        <path d="M16.8999 267H412.529V114.725L314.467 38.5869H16.8999V267Z" fill="url(#backgroundImage)" fill-opacity="0.5" stroke="black" stroke-width="2"/>
        <path d="M412.529 114.725L396.629 77.1377" stroke="black" stroke-width="2"/>
        <text x="30" y="80" fill="black">{text}</text>
        {techStacks.map((tech, index) => (
          <text key={index} x="310" y={140 + index * 20} fill="#5D5C61">{tech}</text>
        ))}
      </svg>
    );
};
